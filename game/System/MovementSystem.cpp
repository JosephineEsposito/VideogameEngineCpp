...
#include "Engine/ENGINE.h"                // for the window's size
#include "Engine/Manager/TimerManager.h"  // for the delta time
...
void MovementSystem::DisplaceUpwardsLeftDiagonal(Vec2& _pos, const Vec2& _vel)
{
...
void MovementSystem::DisplaceUpwardsRightDiagonal(Vec2& _pos, const Vec2& _vel)
{
  _pos.x += _vel.x * TimerManager::GetInstance().GetDeltaTime();
  _pos.y -= _vel.y * TimerManager::GetInstance().GetDeltaTime();
